...
const getOrders = async (req, res, next) => {
  try {
    const orders = await Order.find({state: true}).populate('client').populate('user').populate({path: 'order.product', model: 'Product'}) || [];
...
const getOrderById = async (req, res, next) => {
  try {
    const order = await Order.find({_id: req.params.orderId}).populate('client').populate('user').populate({path: 'order.product', model: 'Product'});
